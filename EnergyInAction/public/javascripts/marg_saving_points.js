$(function () {
  ...
  var savingPoints_data = [];

  var thisMonth_total = [];
  var lastWinter_weekday = [];
  var lastWinter_weekend = [];

  for(var index = 0; index < LAST_WINTER_WEEKDAY.length; index++){
    total = LAST_WINTER_WEEKDAY[index].total;
    lastWinter_weekday.push(Number(total.toFixed(2)));
  }

  for(var index = 0; index < LAST_WINTER_WEEKEND.length; index++){
    total = LAST_WINTER_WEEKEND[index].total;
    lastWinter_weekend.push(Number(total.toFixed(2)));
  }

  var LASTWINTER_WEEKDAY_TOTAL = limitedArraySum(lastWinter_weekday, 96)
  var LASTWINTER_WEEKEND_TOTAL = limitedArraySum(lastWinter_weekend, 96)

  console.log(LASTWINTER_WEEKDAY_TOTAL);
  console.log(LASTWINTER_WEEKEND_TOTAL);
  ...
  // var savingPoints_data = [[12.01, 15], [12.02, 5], [12.03, -15], [12.04, -50], [12.05, -56.5], [12.06, -46.5], [12.07, -22.1], [12.08, -2.5], [12.09, -27.7], [12.1, -55.7], [12.11, -76.5]];
  // savingPoints_data.push([12.12, -36.5]);
  ...
  function drawChart(){
     $('#marg_saving_points').highcharts({
        chart: {
        type: 'spline',
        inverted: true
        },
        title: {
            text: 'MARG Saving Points'
        },
        credits: {
            enabled: false
        },
        ...
        xAxis: {
            ...
            labels: {
                formatter: function () {
                    // console.log(new String(Number(this.value).toFixed(2)).split('.'));
                    date_labels = (new String(Number(this.value).toFixed(2)).split('.'))
                    return date_labels[0] + '월 ' + date_labels[1] + '일';
                    // return this.value;
                }
            },
            maxPadding: 0,
        ...
        series: [{
            name: 'Saving Points',
            // data: [[12.11, 15], [12.10, -50], [12.09, -56.5]]
            data: savingPoints_data
        }]
      });
  };
});
