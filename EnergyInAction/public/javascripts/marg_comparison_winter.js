$(function () {
    ...
    function drawChart(){
      ...
      $('#marg_comparison_winter').highcharts({
        ...
        title: {
           useHTML: true,
           text: '[ 지난 겨울 대비 (' +sign+ (savingRateComparison*100 - 100).toFixed(2) + '%) ]',
           style: {
             color: '#FFFFFF',
             fontWeight: 'bold',
        ...
        series: [
          ...
          {
              name: null,
              data: [{y: Number((lastWinter_totalSum - limitedArraySum(lastWinter_total, todayLength)).toFixed(2)), color: 'lightgray'}, {y: null, color: 'lightgray'}],
              linkedTo: ':previous'
          }, {
              name: '현재 시간 기준 누적 사용량',
              data: [{y: limitedArraySum(lastWinter_total, todayLength), color: comparing_breakdownColors[0]}, {y: limitedArraySum(today_total, todayLength), color: today_breakdownColors[0]}]
          }],
