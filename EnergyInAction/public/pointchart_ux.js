...
$(function () {
  ...
  if(DAY_TO) {
    thisWeek_query = 'api/labs/ux/energy/daily.json?day_from=' + DAY_FROM + '&day_to=' + DAY_TO + '&offset=0';
  } else {
    thisWeek_query = 'api/labs/ux/energy/daily.json?day_from=' + DAY_FROM + '&day_to=' + dateFormatter(new Date(query_yesterday)) + '&offset=0';
  }
  ...
  function thisWeekCB(thisWeek_) {
    ...
    for(var index = 0; index < thisWeek.length; index++){
      ...
      if(isWeekend){
        ...
        console.log("3 point series", (LAST_SEASON_WEEKDAY_COM   - com), (LAST_SEASON_WEEKDAY_LIGHT - light));
        xAxis_categories.push(shortDate)
